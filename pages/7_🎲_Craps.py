...
rolls = st.session_state.craps_rolls
...
def dice_face(n):
    ...
    d = dots.get(n, [])
    circles = "".join(f'<circle cx="{x}" cy="{y}" r="8" fill="#1a1a2e"/>' for x, y in d)
    return f"""<svg width="80" height="80" viewBox="0 0 100 100">
      <rect width="100" height="100" rx="15" fill="white" stroke="#FFD700" stroke-width="4"/>
      {circles}
    </svg>"""
...
if roll_clicked:
    ...
    for _ in range(10):
        ...
        anim_placeholder.markdown(
            f'<div style="display:flex;gap:12px;justify-content:center;margin:10px 0">'
            f'{dice_face(a1)}{dice_face(a2)}</div>',
    ...
    anim_placeholder.markdown(
        f'<div style="display:flex;gap:12px;justify-content:center;margin:10px 0">'
        f'{dice_face(d1)}{dice_face(d2)}</div>',
        unsafe_allow_html=True
    )
