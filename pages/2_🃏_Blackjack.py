...
import random
from db import load_db, save_progress, record_game, unlock_achievement, DEV_ACCOUNTS, MAX_BET
...
def draw_card(): return random.choice(ranks)+random.choice(suits)
...
def render_cards(hand):
    html = ""
    for card in hand:
        suit=card[-1]; rank=card[:-1]
        color="red" if suit in ["♥","♦"] else "black"
        html += f"""<div style="display:inline-block;width:100px;height:140px;border-radius:10px;
        border:2px solid #333;margin:6px;text-align:center;background:white;color:{color};
        font-size:24px;padding-top:10px;"><div>{rank}</div><div style="font-size:40px">{suit}</div></div>"""
...
if st.session_state.bj_result:
    ...
    if r == "win": st.success(f"{msg}  +${amt:,}")
    elif r == "lose": st.error(f"{msg}  -${amt:,}")
    else: st.info("Push — bet returned")
    ...
    with col1:
        if st.button("Hit"):
            ...
            if hand_value(player) > 21:
                ...
                record_game(user, False, st.session_state.bj_bet, 0, "🃏 Blackjack")
                save_progress()
            st.rerun()
    with col2:
        if st.button("Stand"):
            ...
            if dt > 21 or pt > dt:
                ...
                record_game(user, True, st.session_state.bj_bet, st.session_state.bj_bet, "🃏 Blackjack")
            elif pt < dt:
                st.session_state.money -= st.session_state.bj_bet
