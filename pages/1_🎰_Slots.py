...
import random
import time
from db import load_db, save_db, save_progress, record_game, unlock_achievement, DEV_ACCOUNTS, MAX_BET
...
if "slots_bet_input" not in st.session_state:
    ...
    st.session_state["slots_bet_input"] = min(st.session_state["slots_bet_input"], min(money, MAX_BET))
bet = st.number_input("Bet", min_value=1, max_value=min(money, MAX_BET), step=1, key="slots_bet_input")
