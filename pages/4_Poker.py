...
def render_cards(hand, hide=False):
    ...
    for card in hand:
        if hide:
            html += """<div style="display:inline-block;width:90px;height:130px;
            border-radius:10px;border:2px solid #333;margin:6px;text-align:center;
            background:#1a1a2e;color:white;font-size:40px;padding-top:30px;">🂠</div>"""
        else:
            suit = card_suit(card)
            rank = card_rank(card)
            color = "red" if suit in ["♥", "♦"] else "black"
            html += f"""<div style="display:inline-block;width:90px;height:130px;
            border-radius:10px;border:2px solid #333;margin:6px;text-align:center;
            background:white;color:{color};font-size:22px;padding-top:8px;">
            <div>{rank}</div><div style="font-size:36px">{suit}</div></div>"""
...
def score_hand(hand):
    ...
    if freq[0] == 4:
        ...
        kicker = sorted([r for r, c in counts.items() if c != 4], reverse=True)
        return (7, quad) + tuple(kicker)
    ...
    if freq[0] == 3:
        ...
        return (3, trio) + tuple(kickers)
    if freq[0] == 2 and freq[1] == 2:
        pairs = sorted([r for r, c in counts.items() if c == 2], reverse=True)
        kicker = sorted([r for r, c in counts.items() if c == 1], reverse=True)
        return (2,) + tuple(pairs) + tuple(kicker)
    if freq[0] == 2:
...
HAND_NAMES = {9: "Royal Flush", 8: "Straight Flush", 7: "Four of a Kind",
...
st.caption("You vs the Dealer — best 5-card hand from your 2 cards + 5 community cards wins!")
...
# Show result from last round
if st.session_state.th_result:
    outcome, net, pname, dname = st.session_state.th_result
    if outcome == "win":
        st.success(f"🏆 You win! **{pname}** beats dealer's **{dname}** — +${net}")
    elif outcome == "lose":
        st.error(f"Dealer wins with **{dname}** over your **{pname}** — -${net}")
    elif outcome == "fold":
        st.error(f"You folded — lost ${net}")
    else:
        st.info(f"Chop! Both have **{pname}** — bet returned")
...
# -------- BET / DEAL --------
if stage == "bet":
    bet = st.number_input("Ante (your starting bet)", 1, money, min(10, money))
    if st.button("Deal"):
        deck = build_deck()
        ...
        save_progress()
        st.session_state.th_stage = "preflop"
        st.rerun()

# -------- PRE-FLOP --------
elif stage == "preflop":
    st.subheader("Your Hand")
    render_cards(st.session_state.th_player)
    st.subheader("Dealer's Hand")
    render_cards(st.session_state.th_dealer, hide=True)
    ...
    with col1:
        if st.button("✅ Check"):
            st.session_state.th_stage = "flop"
            st.rerun()
    with col2:
        raise_amt = st.number_input("Raise", 1, max(1, st.session_state.money), min(5, max(1, st.session_state.money)), key="r_pre")
        if st.button("💰 Raise"):
...
# -------- FLOP --------
    ...
    render_cards(st.session_state.th_community)
    st.subheader("Dealer's Hand")
    render_cards(st.session_state.th_dealer, hide=True)
    st.write(f"**Pot:** ${st.session_state.th_pot}")
    ...
    with col1:
        if st.button("✅ Check"):
            st.session_state.th_stage = "turn"
            st.rerun()
    with col2:
        raise_amt = st.number_input("Raise", 1, max(1, st.session_state.money), min(5, max(1, st.session_state.money)), key="r_flop")
        if st.button("💰 Raise"):
            st.session_state.th_pot += raise_amt
            st.session_state.th_bet += raise_amt
            st.session_state.money -= raise_amt
...
# -------- TURN --------
    ...
    st.subheader("Your Hand")
    render_cards(st.session_state.th_player)
    st.subheader("Community Cards (Turn)")
    render_cards(community)
    st.subheader("Dealer's Hand")
    render_cards(st.session_state.th_dealer, hide=True)
    st.write(f"**Pot:** ${st.session_state.th_pot}")
    ...
    with col1:
        if st.button("✅ Check"):
            st.session_state.th_stage = "river"
            st.rerun()
    with col2:
        raise_amt = st.number_input("Raise", 1, max(1, st.session_state.money), min(5, max(1, st.session_state.money)), key="r_turn")
        if st.button("💰 Raise"):
            st.session_state.th_pot += raise_amt
            st.session_state.th_bet += raise_amt
            st.session_state.money -= raise_amt
...
# -------- RIVER --------
    ...
    st.subheader("Your Hand")
    render_cards(st.session_state.th_player)
    st.subheader("Community Cards (River)")
    render_cards(community)
    st.subheader("Dealer's Hand")
    render_cards(st.session_state.th_dealer, hide=True)
    st.write(f"**Pot:** ${st.session_state.th_pot}")
    st.info("River — final card. Last chance to act before showdown!")

    col1, col2, col3 = st.columns(3)
    with col1:
        if st.button("✅ Check (Showdown)"):
            st.session_state.th_stage = "result"
            st.rerun()
    with col2:
        raise_amt = st.number_input("Raise", 1, max(1, st.session_state.money), min(5, max(1, st.session_state.money)), key="r_river")
        if st.button("💰 Raise"):
            st.session_state.th_pot += raise_amt
            st.session_state.th_bet += raise_amt
            st.session_state.money -= raise_amt
