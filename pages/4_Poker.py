...
def action_buttons(next_stage, key_suffix):
    ...
    with col1:
        ...
        if st.button(label, key=f"check_{key_suffix}"):
            st.session_state.th_stage = next_stage
            st.rerun()
    with col2:
...
if stage == "bet":
    bet = st.number_input("Ante (your starting bet)", min_value=1, max_value=money,
                          value=min(10, money), step=1)
    if st.button("Deal"):
        deck = build_deck()
    ...
    if player_score > dealer_score:
        ...
        record_game(user, True, st.session_state.th_bet, pot)
        if pname == "Royal Flush":
            unlock_achievement(user, "royal_flush")
    elif player_score < dealer_score:
        st.session_state.th_result = ("lose", st.session_state.th_bet, pname, dname)
        record_game(user, False, st.session_state.th_bet, 0)
    else:
