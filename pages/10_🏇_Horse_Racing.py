...
for i,horse in enumerate(HORSES):
    with cols[i%3]:
        ...
        st.markdown(f"""<div style="background:{bg};border:{border};border-radius:10px;
        ...
        <div style="color:#FFD700;font-size:13px">Odds: {horse['odds']}x</div></div>""",
        unsafe_allow_html=True)
        if st.button("Select", key=f"horse_{i}"):
            st.session_state.race_horse=i; st.rerun()
...
if st.button("🏁 Start Race!", type="primary"):
    ...
    positions=[0]*len(HORSES); race_steps=[]; finished=[False]*len(HORSES); finish_order=[]
        ...
        for i,horse in enumerate(HORSES):
            if not finished[i]:
                ...
                if positions[i]>=TRACK_LENGTH and not finished[i]:
                    finished[i]=True; finish_order.append(i)
        race_steps.append(list(positions))

    track_placeholder = st.empty()
    for step_positions in race_steps[::2]:
        ...
        for i,horse in enumerate(HORSES):
            pos=step_positions[i]; pct=min(int((pos/TRACK_LENGTH)*100),100)
            ...
            track_html+=f"""<div style='margin:6px 0;'>
              <span style='color:{name_color};font-size:12px;display:inline-block;width:130px'>{horse['emoji']} {horse['name'][:10]}</span>
              <span style='display:inline-block;width:{pct}%;max-width:60%;background:{bar_color};height:18px;border-radius:4px;vertical-align:middle;'></span>
              <span style='color:#aaa;font-size:11px;margin-left:4px'>{pct}%</span></div>"""
        track_html+="</div>"
    ...
    for place,hi in enumerate(finish_order):
        horse=HORSES[hi]; is_player=(hi==st.session_state.race_horse)
        name_color="#FFD700" if is_player else "#ffffff"
        medal=["🥇","🥈","🥉","4️⃣","5️⃣","6️⃣"][place]
        final_html+=f"<div style='margin:6px 0;color:{name_color};font-size:14px;'>{medal} {horse['emoji']} {horse['name']}{'  ← YOU' if is_player else ''}</div>"
    final_html+="</div>"
    track_placeholder.markdown(final_html, unsafe_allow_html=True)

    winner_idx=finish_order[0]; locked_bet=st.session_state.race_bet; locked_horse=st.session_state.race_horse
    ...
    if winner_idx==locked_horse:
        ...
        record_game(user,True,locked_bet,payout,"🏇 Horse Racing")
    else:
        st.session_state.money-=locked_bet
        st.session_state.race_result=("lose",f"{HORSES[winner_idx]['name']} won — your horse lost.",locked_bet)
        record_game(user,False,locked_bet,0,"🏇 Horse Racing")
...
for i,horse in enumerate(HORSES):
    with cols[i%3]:
        ...
        <div style="color:#FFD700;font-size:12px">Pays {horse['odds']}x</div></div>""",
        unsafe_allow_html=True)
